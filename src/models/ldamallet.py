...
class LdaMallet(utils.SaveLoad):
    ...
    """
    def __init__(
            self, mallet_path, corpus=None, num_topics=100,
            alpha=50, beta=0.01, id2word=None, workers=4, prefix=None,
        ...
        `prefix` is the string prefix under which all data files
                    will be stored;
                    default: system temp + random filename prefix.

        `optimize_interval` optimize hyperparameters every N iterations
                            (sometimes leads to Java exception;
                            0 to switch off hyperparameter optimization).

        `iterations` is the number of sampling iterations.

        `topic_threshold` is the threshold of the probability above
                            which we consider a topic.
                            This is basically for sparse topic distribution.
        `alpha`  is the topic-doc hyperparameter, float.
            mallet: alpha_k = alpha/num_topics [default in mallet alpha=5]
        `beta` is the word-topic hyperparameter, float
            mallet: beta_w = beta [default in mallet beta = 0.01]
    ...
    def train(self, corpus):
        ...
        cmd = self.mallet_path + " train-topics --input %s --num-topics %s  --alpha %s --beta %s --optimize-interval %s "\
            ...
            "--num-iterations %s --inferencer-filename %s --evaluator-filename %s --doc-topics-threshold %s"
        cmd = cmd % (
            self.fcorpusmallet(), self.num_topics, self.alpha, self.beta, self.optimize_interval, self.workers,
            self.fstate(), self.fdoctopics(), self.ftopickeys(), self.iterations, self.finferencer(), self.fevaluator(), self.topic_threshold)
        ##
        # NOTE "--keep-sequence-bigrams" / "--use-ngrams true" poorer results + runs out of memory
        logger.info("training MALLET LDA with %s", cmd)
        check_output(cmd, shell=True, stderr=subprocess.PIPE)
...
###########################
    def held_out(self, bow, iterations=100, resampling=False):  # default number of iterations:100
        is_corpus, corpus = utils.is_corpus(bow)
        if not is_corpus:
            # query is a single document => make a corpus out of it
            bow = [bow]
        ...
        self.convert_input(bow, infer=True)
        cmd = self.mallet_path + " evaluate-topics --input %s --evaluator %s --output-doc-probs %s --num-particles %s --use-resampling %s"
        cmd = cmd % (self.fcorpusmallet() + '.infer', self.fevaluator(), self.fdocprobs() + '.infer', iterations, resampling)
        logger.info("held-out log probability with MALLET LDA '%s'" % cmd)
        retval = call(cmd, shell=True)
        if retval != 0:
            raise RuntimeError("MALLET failed with error %s on return" % retval)

        result = []
        with utils.smart_open(self.fdocprobs() + '.infer') as fin:
                for lineno, line in enumerate(fin):
                    result += [float(line.split()[0])]  # yield float(line.split()[0])

        # result = list(read_docprobs(self.fdocprobs() + '.infer'))
        return result if is_corpus else result[0]
    ...
    def fdocprobs(self):
        return self.prefix + 'docprobs.txt'
###########################
